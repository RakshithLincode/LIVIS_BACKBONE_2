...
def convert_dict(dict):
	...
	"value_mismatch":dict['ocr_barcode_mismatch'],
	"angle":dict['label_angle'],
	"measurment":dict['label_to_sealent_measurment']}
...
#     #     query.append({'left_frame_status':left_frame_status})
...
#         inspection_id = str(ins['_id'])
#         log_coll = MongoHelper().getCollection(inspection_id+"_results")
#         if status:
#             #print("status")
...
#              #pr = [i["part_name"] = model_selected for i in pr]
#              total_length = total_length + len(pr)
#         for i in pr:
#                 i["model_selected"] = model_selected
#                 i["master_object_id"] = inspection_id
...
#print(query)
	...
	file_name = "D:/BACKEND_HEMLOCK/Hemlock_Backend/datadrive/"+bsf
	...
	fn = file_name + '.csv'
	fn = fn.replace("D:/BACKEND_HEMLOCK/Hemlock_Backend/datadrive/","http://localhost:3306")
	file_w = fn
	# return file_w
